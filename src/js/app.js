...
// Working on adding objects to refactor
let player = {
  health: 3,
  weapon: 1,
  armor: 0,
...
class Enemy {
  ...
  move() {
    // copy movement commands from Player
    //
    // set Interval to trigger it regularly
    //
  }
}
...
function spawnEnemies(amount) {
  ...
  for (let i = 0; i < amount; i++){
    ...
    const enemyType = enemyDefinitions[0];
...
function getEnemyLocation() {
  ...
  while (walls[level].includes(enemyLocation) || enemyLocation === playerLocation || doorLocation === enemyLocation) {
    console.log('preventing spawning in a wall');
